...
module.exports = {
  ...
  exits: {
    ...
    notFound: {
      outputFriendlyName: 'Helper not found',
      outputDescription: 'Helper required to validate email was not found',
    },
  },
  ...
  fn: async function ({
    ...
    if (!sails.hooks.views) {
      throw new Error('Hook `views` is required to process templates');
    }
    ...
    // Define mail process adapter
    let adapter = sails.config.email.adapter || 'log';

    // Check if email address is valid
    // @TODO: Check from and BCC emails?
    let helperHandler = await sails.helpers.email
      .getHelper(sails.config.email.validateEmailHelper)
      .intercept('notFound', 'notFound');

    // @TODO: Check if function is either async or sync
    const isValidEmailAddress = await helperHandler(to);

    // If false then the email address is fake
    const isToAddressConsideredFake = !isValidEmailAddress;
    ...
    // If no email to send is required use log as adapter
    if (dontActuallySend && adapter !== 'log') {
      adapter = 'log';
    }

    // Check is helper is loadaded from sails-hook-organics
    if (
      _.includes(['mailgun', 'sendgrid'], adapter) &&
      _.isEmpty(sails.hooks.organics)
    ) {
      sails.log.warn(
    ...
    // Move all settings to dedicated sails.config.email
    from = _.isString(from) ? from : sails.config.email.fromEmailAddress;
    fromName = _.isString(fromName) ? fromName : sails.config.email.fromName;

    // Add a prefix if the environment is not production
    ...
    const sendWith = _.camelCase(`send-${adapter}`);

    const deferred = sails.helpers.email[sendWith].with(messageData);
    if (ensureAck) {
      await deferred;
    } else {
