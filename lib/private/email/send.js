...
module.exports = {
  ...
  fn: async function ({
    ...
    const isValidEmailAddress = _.isFunction(sails.config.email.isValidEmail)
      ? sails.config.email.isValidEmail
      : undefined;

    // If false then the email address is fake
    const isToAddressConsideredFake = !_.isUndefined(isValidEmailAddress)
      ? !Boolean(isValidEmailAddress(to))
      : false;
    ...
    const helper = _.camelCase(`send-${adapter}`);

    const deferred = sails.helpers.email[helper].with(messageData);
    if (ensureAck) {
      await deferred;
    } else {
